...
def main():
    ...
    print("using {} images for training, {} images for validation.".format(train_num,
                                                                           val_num))
    # net = torchvision.models.densenet121(num_classes=1000)
    # net = models.densenet121(weights=models.DenseNet121_Weights.IMAGENET1K_V1)
    # num_ftrs = net.classifier.in_features
    # net.classifier = nn.Linear(num_ftrs, 257)
    ...
    for epoch in range(epochs):
        ...
        if val_accurate > best_acc:
            ...
            with open('training_results_caltech.txt', 'a') as file:
                file.write(
        ...
        plt.ylabel('Loss')
        plt.legend()
        plt.savefig('(Caltech-256)Loss_Plot.png')  # 保存图像到文件
        plt.close()
        ...
        plt.ylabel('Accuracy')
        plt.legend()
        plt.savefig('(Caltech-256)Accuracy_Plot.png')  # 保存图像到文件
        plt.close()  # 释放资源
